...
gain_absolu = lambda pos, prix_actuel: (
    ...
    if pos.purchase_price > 0 else 0
)
...
poids_portfolio = lambda pos, prix_actuel, total: (
    (valeur_actuelle(pos, prix_actuel) / total) * 100
    if total > 0 else 0
)
...
def afficher_positions(positions):
    ...
    total_valeur_actuelle = sum(
        valeur_actuelle(p, prix_actuels.get(p.symbol, p.purchase_price))
        for p in positions
    )
